...
export default function EmployementDetail(props) {
    ...
    useEffect(() => {
        httpService.all([
            ...
            const [billingStatus, taxType, workAuthorization, designations, departments, wageCycle, vendors] = responses;
    ...
    const getReportingTo = (departmentId) => {
        httpService.get(`${API_PATH.GET_REPORTINGTO}${departmentId}`).then(res => {
            const { data } = res;
    ...
    const getProjects = (vendorId) => {
        httpService.get(`${API_PATH.GET_PROJECT}${vendorId}`).then(res => {
    ...
    const onSubmitHandler = () => {
        ...
        if (result === true) {
            props.setemployeeDetailData(account);
        }
    }
    return (
        <Grid container spacing={2}>
            ...
            <Grid item xs={3}>
                ...
                <Autocomplete
                    ...
                    fullWidth
                    renderInput={(params) => {
                        return (<TextField  {...params} error={!stateObj.error.vendorsTier2.errorObject.isValid}
                            helperText={stateObj.error.vendorsTier2.errorObject.errorMessage} name="vendorsTier2" margin="normal" inputProps={{
                                ...params.inputProps,
                                autoComplete: 'new-password', // disable autocomplete and autofill
                            }} fullWidth InputLabelProps={{
                                shrink: true,
                            }} label="Select Vendor Tier 2" variant="outlined" />)
                    }}
                />
            </Grid>
